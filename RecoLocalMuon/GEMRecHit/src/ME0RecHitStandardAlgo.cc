				    ...
				    LocalError& error)  const
{
  ...
  float fstrip = (roll.centreOfStrip(cluster.firstStrip())).x(); // 20022018
  float lstrip = (roll.centreOfStrip(cluster.lastStrip())).x(); // 20022018
  float centreOfCluster = (fstrip + lstrip)/2; // 20022018

  LocalPoint loctemp2(centreOfCluster,0.,0.);
 
  Point = loctemp2;
  error = roll.localError((cluster.firstStrip()+cluster.lastStrip())/2., cluster.clusterSize()); // 20022018
  return true;
}
